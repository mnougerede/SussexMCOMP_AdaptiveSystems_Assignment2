...
distinct timescales. A \emph{falling-shape environment} presents circular
objects that descend through the agent's field of view. An \emph{agent body},
...
\emph{continuous-time recurrent neural network} (CTRNN) maps sensor readings to
motor commands. A \emph{homeostatic plasticity} (HP) module adjusts the
...
are fixed across all runs. The experimental conditions vary \emph{when} HP acts,
...
\begin{figure}[t]
  ...
  generations. \emph{[Placeholder --- figure to be inserted.]}}
  \label{fig:system}
\end{figure}
...
experiments and are noted as future work
(Section~\ref{sec:discussion}).\footnote{The CTRNN integration was performed
using the \texttt{madvn/CTRNN} package \parencite{candadai2020}, whose
algorithm is exactly Equations~\ref{eq:ctrnn-state}--\ref{eq:ctrnn-euler}; it is
described here as our own per the assignment requirements. Analysis-only tools
(\texttt{numpy}, \texttt{scipy}, \texttt{matplotlib}) are identified in
...
\end{equation}
%
The absolute value $\lvert w_{ji} \rvert$ in Equation~\ref{eq:hp-weight} is
essential: it ensures that both excitatory and inhibitory weights grow in
\emph{magnitude} when $\rho_i > 0$ and shrink when $\rho_i < 0$, so that it is
the strength of a connection that is regulated, not its sign. Equations
...
\emph{developmental period} of $6000$ timesteps during which the network
...
% [CHECK] Confirm the exact eq.7.3 components and the S_max numerator/denominator
% against the thesis before final submission. Drafted from methods_log + todo.
...
combines the reduction in distance with the final proximity, normalised by a
per-shape maximum
%
\begin{equation}
...
and vertical velocities, so that faster-falling or laterally-moving shapes
(which are harder to catch) scale the score appropriately. The
displacement-reduction contribution is clipped at zero so that an agent which
increases its distance scores no worse than one that does not move. The trial
fitness is the mean per-shape score over the shapes in the trial, and an
individual's fitness is the mean over several independently-seeded trials to
average out the randomised shape sequences. % [CHECK] number of shapes per trial
% and trials per evaluation used in the final runs (todo says 20 shapes; trials
% per eval = 3 in the runs, despite earlier "10 trials" planning note).
...
to produce each offspring, three individuals are drawn at random and the fittest
is chosen as a parent. A single elite individual (the current best) is copied
unchanged into the next generation. There is no crossover. Mutation is applied
per allele with probability $p_m = 0.03$; a mutated allele is perturbed by
Gaussian noise $\mathcal{N}(0, \sigma_m^2)$ with $\sigma_m = 0.1$, and values
that fall outside $[-1, 1]$ are reflected back into range.

This GA differs deliberately from Williams's, which used rank-based
roulette-wheel selection with an elite of five and a mixed mutation scheme.
Tournament selection with a single elite is the standard choice in evolutionary
computation and avoids the rapid loss of diversity that roulette-wheel selection
with a large elite can cause; the consequences of this choice for comparison
with Williams are taken up in the discussion
(Section~\ref{sec:discussion}). The per-allele mutation rate matches Williams's
$3\%$.
...
%     copy the single fittest individual to the next generation (elitism)
%     while next generation not full:
%       parent <- argmax fitness of K=3 random individuals
%       child  <- copy(parent)
...
data rather than live runs. Analysis tooling is \texttt{numpy},
\texttt{scipy} (statistics), \texttt{matplotlib} (figures) and \texttt{pandas};
these are used only for analysis and so are not described at the algorithmic
level.
...
individual is evaluated twice: once with HP active and once with HP frozen by
adiabatic elimination. For \emph{Behaviour only}, HP runs for an initial
settling window of the first ten shapes and is then frozen, with fitness
measured over the remaining shapes; for \emph{Both}, HP is frozen at the end of
the developmental period and fitness is measured over the trial. \emph{Dev only}
and \emph{No HP} serve as controls, for which the frozen variant should produce
no change because HP was already inactive during their behaviour. To establish
the measurement noise against which a fitness drop is judged significant, each
individual is additionally re-evaluated $20$ times with different shape-sequence
seeds and HP active; a frozen-HP drop is reported in units of this
...
spread serves as a diversity proxy; genotype-level diversity is not recovered
because only population fitnesses, not genotypes, are retained per generation.
