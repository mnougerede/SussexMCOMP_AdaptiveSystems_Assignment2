...
% [CHECK] Verify the per-shape score formula (eq:per-shape-score) against
% src/environment/fitness.py before submitting.
...
final distance at the moment the shape passes the agent. The per-shape score is
%
\begin{equation}
  c \;=\; \frac{\max(D_0 - D_f,\; 0) + \max(D_0 - D_f,\; 0)}{S_{\max}},
  \label{eq:per-shape-score}
\end{equation}
%
normalised by a per-shape maximum that accounts for shape speed,
%
\begin{equation}
...
(which are harder to catch) scale the maximum score appropriately. The
