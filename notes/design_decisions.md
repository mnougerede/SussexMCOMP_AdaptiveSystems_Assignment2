...
**Empirical result (8c complete).** Genetic assimilation has not occurred in any HP condition. All 15 HP individuals show fitness drops significantly exceeding the noise baseline (No HP controls: ±0.85 SD). Dev only: drops of 5.6–21.2 SD; two individuals (s203, s600) collapse to frozen fitness = 0.000 — the raw genotype drives a near-maximum motor asymmetry (z[3]=0.969, z[4]=0.012) that the developmental HP phase rescues every generation; without it the agent accelerates to body.x = −25,450 and catches nothing. Behaviour only: 1.0–5.3 SD, partial collapse, no complete failures. Both: bimodal — two complete collapses (s401, s402) and three moderate drops. Full results in `figs/frozen_hp_results.csv` and methods\_log §11.5.
