...
**Empirical result (8c complete, n=10).** Genetic assimilation has not occurred in any HP condition. All 30 HP individuals show fitness drops significantly exceeding the noise baseline (No HP controls: all within ±1 SD). Dev only: 5/10 collapses to frozen fitness = 0.000 (drops 16.8–28.8 SD); 5/10 large partial drops (5.6–8.6 SD). Behaviour only: bimodal at n=10 — 3/10 complete or near-complete collapses, 4/10 moderate drops, 2/10 small drops; the n=5 characterisation of Behaviour only as "moderate drops only" was not representative. Both: 3/10 complete collapses, 4/10 moderate, 1/10 near-zero. Full results in `figs/frozen_hp_results.csv` and methods\_log §11.5.
...
**Implementation note:** the replication-versus-extension split is a presentation boundary, not a data boundary. Both halves read the same `history/` arrays, so in code this is most likely one data-loading layer feeding two figures, and probably the same script that produces the Phase 7 replication figure. The split governs where results live in the report, not how many things get built.

**What the figure shows (first batch, 5 runs/condition).** The Dev only condition jumps to a high fitness within roughly the first 5 generations and then plateaus, whereas the other three improve gradually across all 200 generations. Population fitness spread collapses to near zero in all four conditions by roughly generation 10 to 20 and stays flat. The reading is that all conditions converge early and on a similar timescale, and the conditions differ in *where* they converge rather than in whether or how fast. Dev only converges to a higher optimum, consistent with the developmental phase pre-conditioning each individual before evaluation. The Both puzzle (Both runs the same developmental phase as Dev only but its fitness collapses to the No HP baseline) and the Dev only paradox (best-performing condition yet most HP-dependent) are resolved by the 8b and 8c findings; see methods\_log §11.5 for the full mechanistic interpretation. The early convergence is not treated as premature-convergence pathology: with population 30 on a 35-parameter genotype, sustained diversity was not expected, and fitness keeps improving after the spread collapses, indicating refinement within a converged gene pool rather than a stalled search.
