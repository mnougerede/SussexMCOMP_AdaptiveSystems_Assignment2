...
**What the figure shows (first batch, 5 runs/condition).** The Dev only condition jumps to a high fitness within roughly the first 5 generations and then plateaus, whereas the other three improve gradually across all 200 generations. Population fitness spread collapses to near zero in all four conditions by roughly generation 10 to 20 and stays flat. The reading is that all conditions converge early and on a similar timescale, and the conditions differ in *where* they converge rather than in whether or how fast. Dev only converges to a higher optimum, which is consistent with the developmental phase pre-conditioning each individual before evaluation (the Baldwin setup). The full interpretation, including the open puzzle that Both does not inherit Dev only's head start despite running the identical developmental phase, is recorded in `notes/methods_log.md` §11.4 and is flagged there as a hypothesis awaiting the 8b and 8c results. Do not assert the mechanism in the report before those analyses are in. The early convergence is not treated as premature-convergence pathology: with population 30 on a 35-parameter genotype, sustained diversity was not expected, and fitness keeps improving after the spread collapses, indicating refinement within a converged gene pool rather than a stalled search.
